...
#'
#' Calculates focal ("moving window") weight matrix using \code{translateRisk2D} for 
...
#' @seealso \code{translateRisk2D} for examples
#' @export
focalWeightRisk <- function(x, type, a, b, riskCutoff=0.01){
  ...
  risktype <- type
  wtmat <- matrix(translateRisk2D(as.vector(distmat),type=risktype, a=a, b=b, riskCutoff=riskCutoff), nrow=size, ncol=size)
